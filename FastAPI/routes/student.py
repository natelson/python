...
#update a student
@student_router.put('/studente/{studentId}')
async def update_student(studentId, student: Student):
    #find the student and than update it whith new student data
    connection.local.student.find_one_and_update(
        {"_id": ObjectId(studentId),
         "$set": dict(student)
        }
    )
    return connection.local.student.find_one({"_id": ObjectId(studentId)})
